...
class BlastFileProcessor(object): 
    ...
    def GetBH_s(self, pairwiseScoresMatrices, iSpecies):
        ...
        H = [None for i_ in range(self.nSpecies)] # create array of Nones to be replace by matrices
    ...
    def GetBLAST6Scores(self, iSpecies, jSpecies): 
        ...
        row = ""
        try:
            with open(self.filesDirectory + "Blast%d_%d.txt" % (self.speciesToUse[iSpecies], self.speciesToUse[jSpecies]), 'r') as blastfile:
                blastreader = csv.reader(blastfile, delimiter='\t')
                for row in blastreader:    
                    # Get hit and query IDs
                    try:
                        species1ID, sequence1ID = list(map(int, row[0].split(self.sep, 1))) 
                        species2ID, sequence2ID = list(map(int, row[1].split(self.sep, 1)))     
...
class WaterfallMethod:
    ...
    def RunWaterfallMethod(self, graphFilename):
        ...
        for iSpecies in range(self.thisBfp.nSpecies):
            Bi = []
            for jSpecies in range(self.thisBfp.nSpecies):
        ...
        with open(graphFilename, 'w') as graphFile:
            ...
            graphFile.write("\n(mclmatrix\nbegin\n\n")  
            for iSpec in range(self.thisBfp.nSpecies):
                # calculate the 2-way connections for one query species
                connect2 = []
                for jSpec in range(self.thisBfp.nSpecies):
                ...
                for query in range(self.thisBfp.NumberOfSequences(iSpec)):
    ...
    def GetMostDistant_s(self, RBH, B, iSpec):
        ...
        for kSpec in range(self.thisBfp.nSpecies):
...
def AssignIDsToSequences(fastaDirectory, outputDirectory):
    ...
    if os.path.exists(speciesFilename):
        with open(speciesFilename, 'r') as infile:
    ...
    returnFilenames = []
    newSpeciesIDs = []
    with open(idsFilename, 'a') as idsFile, open(speciesFilename, 'a') as speciesFile:
        for fastaFilename in originalFastaFilenames:
            ...
            outputFasta = open(outputFastaFilename, 'w')
            ...
            with open(fastaDirectory + os.sep + fastaFilename, 'r') as fastaFile:
...
def CreateOrthogroupTable(ogs, 
    ...
    with open(speciesFilename, 'r') as speciesNamesFile:
    ...
    with open(outputFilename, 'w') as outputFile, open(singleGeneFilename, 'w') as singleGeneFile:
        ...
        for iOg, (og, og_names) in enumerate(zip(ogs_ints, ogs_names)):
            ...
            if len(og) == 1:
                rows.extend(['' for x in range(nSpecies)])
...
#            workingDir_previous = GetDirectoryArgument(arg, args)
    ...
    if qXML:   
        ...
        for fastaFilename, iSpecies in list(fastaFileIndices.items()):
